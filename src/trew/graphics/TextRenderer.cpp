...
void TextRenderer::set_geometry_data(Context* context, GeometryData* geometry_data)
{
...
void TextRenderer::transfer_data(Context* context, GeometryData* geometry_data)
{
    SDL_GPUCopyPass* copy_pass = SDL_BeginGPUCopyPass(context->cmd_buf);
...
void TextRenderer::draw(Context* context, SDL_Mat4X4* matrices, int num_matrices, TTF_GPUAtlasDrawSequence* draw_sequence)
{
    SDL_GPUTexture* swapchain_texture;
    SDL_WaitAndAcquireGPUSwapchainTexture(context->cmd_buf, context->window, &swapchain_texture, NULL, NULL);
    ...
    if (swapchain_texture != NULL) {
        ...
        colour_target_info.clear_color = { 0.3f, 0.4f, 0.5f, 1.0f };
...
void TextRenderer::DOIT()
{
    ...
    context.window = SDL_CreateWindow("GPU text test", 800, 600, 0);

    context.device = SDL_CreateGPUDevice(SUPPORTED_SHADER_FORMATS, true, NULL);
    ...
    TTF_Init();
    TTF_Font* font = TTF_OpenFont(font_filename, 50); /* Preferably use a Monospaced font */
    if (!font) {
        running = false;
    }
    ...
    TTF_TextEngine* engine = TTF_CreateGPUTextEngine(context.device);
